document.addEventListener("DOMContentLoaded", function() {
	...
	const menuButtonBack = document.querySelector('.js-menu-back');
	...
	menuButtonBack.addEventListener('click', function(e) {
		document.querySelector('.popup-menu-wrap li.open').classList.remove('open')
		e.preventDefault()
		return false
	})
	...
	function updateColors() {
		const containers = document.querySelectorAll('.items-colors');
		if (containers) {
			containers.forEach(container => {
				const allItems = container.querySelectorAll('.item-wrap');
				allItems.forEach(item => {
				  item.classList.remove('item-odd', 'item-even');
				});
				const activeItems = container.querySelectorAll('.item-wrap.active');
				activeItems.forEach((item, index) => {
				  item.classList.add(index % 2 === 0 ? 'item-odd' : 'item-even');
				});
			  });
		}
	  }
	...
	class BeforeAfterSlider {
		constructor(container) {
			...
			this.afterWrapper = container.querySelector('.comparison-after-wrapper');
			this.handle = container.querySelector('.comparison-handle');
			this.ctitle = container.querySelector('.comparison-title-after');
		...
		setPosition(percent) {
			...
			this.afterWrapper.style.width = widthPercent + '%';
			this.handle.style.left = widthPercent + '%';
			this.ctitle.style.left = widthPercent + '%';
		}
	}
	...
	sliderstiles.forEach((container) => {
		...
		// Получаем начальный слайд из атрибута data-start
		let initialSlide = 0;
		if (container.dataset.start !== undefined) {
			const startValue = parseInt(container.dataset.start, 10);
			if (!isNaN(startValue) && startValue >= 0) {
				initialSlide = startValue;
			}
		}
